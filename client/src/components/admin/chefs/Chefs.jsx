...
const Chefs = () => {
    ...
    const fetchAllChefs = async() => {
        try{
            ...
            const response = await axios.get(`${BASEURL}/api/fetchchefs?page=${page}&limit=5`);
    ...
    const deleteChef = (e, id) => {
        ...
        if(answer){
            ...
            axios.delete(`${BASEURL}/api/delete-chef/${id}`).then(res => {
    ...
    if(chefs.length === 0){
         ...
         ChefList_HTML = <tr>
             <td className="no-data-available" colSpan="7">There are no registered Chefs</td>
         </tr>
    }else{
